    ...
    for name, data in rss_saver.items():
        try:
            ...
            while True:
                ...
                parse = True
                for flist in data[3]:
                    if not any(x in str(rss_d.entries[feed_count]['title']).lower() for x in flist):
                        parse = False
                        feed_count += 1
                        break
                if not parse:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        # Save the torrent file to a specific directory
                        download_dir = "torrent_files"
                        os.makedirs(download_dir, exist_ok=True)

                        file_name = os.path.join(download_dir, f"{name}_{feed_count}.torrent")
...
def rss_download(update, context):
    try:
        ...
        if response.status_code == 200:
            ...
            sendMessage(f"Torrent file downloaded: {file_name}", context.bot, update.message)
        else:
            sendMessage("Failed to download the torrent file.", context.bot, update.message)
    except IndexError:
        sendMessage(f"Use this format to download the torrent:\n/{BotCommands.RssDownloadCommand} Torrent_URL", context.bot, update.message)
    except Exception as e:
        sendMessage(str(e), context.bot, update.message)
...
if DB_URI is not None and RSS_CHAT_ID is not None:
    ...
    rss_download_handler = CommandHandler(BotCommands.RssDownloadCommand, rss_download, filters=CustomFilters.owner_filter | CustomFilters.sudo_user, run_async=True)

    dispatcher.add_handler(rss_download_handler)
